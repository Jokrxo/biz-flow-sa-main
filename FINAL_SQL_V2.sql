...
-- Version 2 - Handles missing tables gracefully

-- 1. Create audit_logs table first (no dependencies)
...
-- 2. Create log_audit_event function (for Customer audit trail)
...
-- 9. Add missing columns to customers (wrap in DO to handle errors)
DO $$
BEGIN
...
-- 10. Add fiscal columns to app_settings
DO $$
BEGIN
...
-- 11. Create ensure_user_company_and_role function (doesn't need chart_of_accounts)
...
-- 12. Create get_next_invoice_number function (only needs invoices table)
...
-- 13. Create can_soft_delete_customer function (uses try-catch for missing tables)
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'chart_of_accounts') INTO v_has_table;
  
  IF NOT v_has_table THEN
    RETURN TRUE; -- Allow deletion if table doesn't exist
  END IF;
...
-- 14. Create create_customer_ledger_entry function
...
-- 15. Create recalculate_customer_balance function
CREATE OR REPLACE FUNCTION recalculate_customer_balance(p_customer_id UUID)
RETURNS NUMERIC
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE v_balance NUMERIC; v_company_id UUID; v_has_table BOOLEAN;
BEGIN
  ...
  RETURN v_balance;
EXCEPTION WHEN undefined_table THEN
  UPDATE customers SET balance = 0, updated_at = NOW() WHERE id = p_customer_id;
  RETURN 0;
END;
$$;
