-- SUPER_ULTIMATE_SQL.sql
-- Completely standalone - NO foreign key dependencies on tables we create
-- Run this FIRST before any other SQL

-- 1. Create companies table (standalone)
...
-- 2. Create chart_of_accounts table (standalone - NO foreign key)
CREATE TABLE IF NOT EXISTS chart_of_accounts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  company_id UUID,  -- No FK reference to avoid dependency issues
...
-- 3. Create transactions table (standalone)
...
-- 4. Create ledger_entries table (standalone)
...
SELECT 'SUPER_ULTIMATE_SQL completed! All tables created successfully.' as result;
